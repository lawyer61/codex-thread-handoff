...
- `SubagentStart`：为 child agent 建立 execution lane，并默认注入 parent thread 的 bounded handoff；可单独关闭注入。
- `UserPromptSubmit`：记录用户新要求；可配置为在“继续/接着刚才”等场景按需注入上下文，默认不注入。
- `PostToolUse`：记录工具调用和结果摘要，包括 `spawn_agent`、`wait_agent` 等 multi-agent 控制工具。
- `Stop`：不阻塞当前 Codex 流程；默认不触发 summarizer，可配置为在 handoff 过期时后台触发 summarizer。
- `SubagentStop`：标记 child lane 完成并记录经过脱敏、带 agent 归属的最终结果；不会调用 summarizer。
- `PreCompact`：root agent compact 前最多等待 summarizer 一个有界时间，默认 8 秒；child agent 不调用共享 summarizer。无论成功、失败或超时都允许 compact 继续。
- `PostCompact`：root compact 推进 root context epoch 并重建注入 brief；child compact 只推进对应 lane 的 epoch。

插件维护四类数据：

- `sessions/<sha256(session_id)>.json`：把每个 Codex session 绑定到自己的 logical thread，避免同一项目中的并行客户端互相覆盖。
- `state.json` 中的 `agent_lanes`：记录同一 root session 下各 child agent 的 id、类型、transcript、状态和独立 context epoch。
- `events.jsonl`：事件账本，记录用户提示、工具观察、subagent 生命周期、compact 边界、summary job 状态，以及 `turn_id` / `agent_id` / `agent_type` 等归属信息。
- `latest.md` / `latest.inject.md`：由 summarizer 生成的人类可读 canonical handoff 和用于注入的 bounded brief。

`active_thread` 只保留为旧版本迁移和最近创建 thread 的兼容指针。已有 session 的后续 hooks 优先读取 session binding，不再依赖这个项目级指针。

Codex multi-agent 模式下，同一个 root `session_id` 派生的 children 共享一个 logical thread，但每个 `agent_id` 使用独立 execution lane。child prompt、tool result、compact boundary 和最终结果都会保留 agent 归属。多个互不相关的顶层 Codex session 仍由 session binding 隔离。
...
Child agent 不会各自调用 summarizer。`SubagentStop` 只把 child 最终结果作为带归属的 evidence 写入账本，下一次 root `PreCompact` 再把这些 child findings 合并进 canonical handoff。
...
同一目录可以同时运行多个 Codex 客户端。每个不同的 `session_id` 默认使用独立 logical thread、事件账本和 handoff；`clear` 或“新任务”只会重新绑定当前 session。使用同一 `transcript_path` 恢复的 session 会继续原 logical thread。

开启 Codex multi-agent 后，child 默认会在 `SubagentStart` 收到当前 `latest.inject.md`。这让新 child 能看到已有约束和任务状态，同时仍使用自己的 transcript 和 context epoch。
...
7. root agent 启动 child 时，`SubagentStart` 默认把同一 bounded handoff 注入 child；child 完成后，`SubagentStop` 只记录结果，不额外消耗 summarizer token。
...
默认情况下，`SubagentStart` 会向 child agent 注入 handoff。若只想记录 child 生命周期和结果、不向 child 注入：
...
- 后台 summary job 会携带创建它的 logical thread id，即使 session 随后切换到新任务，旧 job 也不会写入新 thread。
- PreCompact 永远返回 `continue:true`。
- Child PreCompact 不调用外部 summarizer；Child PostCompact 不修改 root context epoch。
- SubagentStop 永远不调用 summarizer，只记录经过脱敏的 attributed completion evidence。
...
### 同一目录中的多个 Codex 任务互相污染

`v0.3.7` 及更早版本使用项目级 `active_thread` 路由所有 hooks，因此多个独立 Codex session 可能写入同一个 `events.jsonl` 和 `latest.md`。`v0.3.8` 起，每个 `session_id` 都有独立 binding；旧数据会根据 `codex_sessions`、`transcript_path` 和 legacy `active_thread` 自动迁移。如果一个旧 thread 已经混入多个 transcript 不同的 session，首先恢复的 lineage 会保留旧 thread，其他 session 会创建新 thread，避免继续交叉写入。

### Multi-agent child 没有收到 handoff

Multi-agent lifecycle 支持从 `v0.4.0` 开始。确认已经升级并在 `/hooks` 中 trust `SubagentStart` / `SubagentStop`。默认 `THREAD_HANDOFF_INJECT_ON_SUBAGENT_START=true`；如果显式设为 `false`，child 仍会被记录，但不会收到 `latest.inject.md`。
...
- 如果 hook 输入缺少 `session_id`，插件只能回退到 legacy `active_thread`，此时无法保证同目录并行任务隔离。
- 当前 hook payload 没有提供 parent-agent id，因此 nested subagents 会保留各自 `agent_id`，但 `agent_lanes` 不记录完整父子树。
- External summarizer 不直接读取每个 child transcript tail；它使用带归属的 child prompt/tool events 和 `SubagentStop` 最终结果，在下一次 root summary job 中合并 child findings。
